...
namespace PriceGateway.BLL
{
    public class CPriceGateway : IPriceGateway
    {
        ...
        private readonly Dictionary<string, string> _channelHubMapping;

        public CPriceGateway(IS6GApp s6GApp, Lazy<ConnectionMultiplexer> redis, Lazy<ConnectionMultiplexer> redis_Sentinel, IConfiguration configuration, IHubContext<Hub_HSX, IHubClient> hubClient_HSX, IHubContext<Hub_HNX, IHubClient> hubClient_HNX, IServiceProvider serviceProvider) 
        {
            this._s6GApp = s6GApp;
            ...
            _channelHubMapping = _configuration.GetSection("RedisChannels").Get<Dictionary<string, string>>();
        }
        public void StartListeningToRedisChannel()
        {
            var subscriber = _redis.GetSubscriber();

            foreach (var mapping in _channelHubMapping)
            {
                var channel = mapping.Key;
                subscriber.Subscribe(channel, (chan, message) =>
                {
                    var hubPath = _channelHubMapping[chan];
                    PublishToHub(hubPath, message.ToString());
                });
            }
        }

        private async void PublishToHub(string hubPath, string message)
        {
            var hubContextType = typeof(IHubContext<GenericHub, IHubClient>);
            var hubContext = (IHubContext<GenericHub, IHubClient>)_serviceProvider.GetRequiredService(hubContextType);

            await hubContext.Clients.All.ReceiveMessage(hubPath, message);
        }
        //public void StartListeningToRedisChannel()
        //{
        //    try
