...
{
    [Route(CPriceConfig.__ROUTE_API_GET_BASKET)]  //Định nghĩa route của api
    [ApiController]
    public class ApiGetBasket : Controller
    {
        public readonly IS6GApp _cS6GApp;
        private readonly IPriceHandle _handle;

        public ApiGetBasket(IPriceHandle handler, IS6GApp cS6GApp)
        {
            this._handle = handler;
            this._cS6GApp = cS6GApp;
        }
        [HttpGet]
        public async Task<IActionResult> Api_Get_Basket(string exchange)
        {
            try
            {
                EResponseResult responseResult = await this._handle.fnc_Get_Basket(exchange);
